...
Отправьте мне шифр специальности (например: "070107 3"), и я найду все подходящие образовательные программы.
...
1. Отправьте шифр специальности (например: "070107 3")
2. Бот найдет все соответствующие образовательные программы
3. Получите список в формате: "Код программы - Название программы"
...
async def search_cipher(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if results:
        ...
        monospace_results = "\n".join(results)
        response += f"```\nВы можете поступить в наш университет по данным ГОП:\n{monospace_results}\n \nТак же выпускникам колледжа у нас есть 30% скидка если подаёте на обучение на платной основе. Для этого вам нужно будет подойти в универ и подать заявку. Скидка слетает при неуплате или задолженности.```"
        ...
        response += "Проверьте правильность написания шифра или обратитесь к администратору."
